...
Aplika is built by two developers, with AI agents contributing code and commits. Without an explicit contract, commit messages drift, history becomes hard to scan, and review and changelog automation lose their input. We also run a two-stage delivery flow — production and a pre-production testing branch — and we practice TDD, which changes how work is naturally split into commits.
...
`ui` is a Aplika extension, not part of the base Conventional Commits set. Commit tooling (e.g. `commitlint`) MUST be configured to accept it; until that exists, reviewers enforce it.
...
- Follow `work-unit-commits` for reviewable units; this ADR specializes it for Aplika.
...
- [Aplika readme — Development Workflow](../readme.md#development-workflow)
- [Aplika readme — TDD](../readme.md#tdd)
